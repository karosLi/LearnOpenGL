...
int main()
{
    ...
    // ------------------------------------
    Shader lightingShader("opengl.vs", "opengl.fs");
    ...
    unsigned int emissionMap = loadTexture("matrix.jpeg");
    
    lightingShader.use();
    // 绑定纹理单元0到 uniform 采样器变量
    lightingShader.setInt("material.diffuseTexture", 0);
    
    // 绑定纹理单元1到 uniform 采样器变量
    lightingShader.setInt("material.specularTexture", 1);
    
    // 绑定纹理单元2到 uniform 采样器变量
    lightingShader.setInt("material.emissionTexture", 2);
...
//        lightingShader.setVec3("light.specular", 1.0f, 1.0f, 1.0f);
//
//        // material properties 金色材质，材质清单：http://devernay.free.fr/cours/opengl/materials.html
