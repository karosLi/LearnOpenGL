...
struct Material {
    ...
    sampler2D emissionTexture;// 放射光贴图
};
...
//    vec3 specular = light.specular * spec * (vec3(1.0) - vec3(texture(material.specularTexture, TexCoords)));
    ...
    /// 放射光
    //  放射光贴图(Emission Map)的东西，它是一个储存了每个片段的发光值(Emission Value)的贴图。发光值是一个包含（假设）光源的物体发光(Emit)时可能显现的颜色，这样的话物体就能够忽略光照条件进行发光(Glow)。游戏中某个物体在发光的时候，你通常看到的就是放射光贴图（比如 机器人的眼，或是箱子上的灯带）
    // emission
    vec3 emission = vec3(texture(material.emissionTexture, TexCoords));
