...
int main()
{
    ...
    // configure depth map FBO - 配置深度立方体贴图帧缓冲
    // -----------------------
    const unsigned int SHADOW_WIDTH = 1024, SHADOW_HEIGHT = 1024;
    unsigned int depthMapFBO;
    glGenFramebuffers(1, &depthMapFBO);
    // create depth cubemap texture - 创建深度立方体贴图纹理
    ...
    // attach depth texture as FBO's depth buffer - 把深度立方体贴图纹理附加到帧缓冲的深度附件上
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 0. create depth cubemap transformation matrices - 以点光源为原点创建6个 阴影投影矩阵 * 视图矩阵（摄像机lookAt），每个都按顺序注视着立方体贴图的的一个方向：右、左、上、下、近、远，从而得到6个不同的光空间变换矩阵，方便转换世界坐标到以点光源为原点的6个视椎体裁剪空间内
        // -----------------------------------------------
        /**
         光空间的变换
        ...
        // 1. render scene to depth cubemap - 首先我们生成深度立方体贴图
        // --------------------------------
        ...
        // 现在 depthCubemap 深度立方体贴图存储着以点光源为中心的6个面的深度值
        renderScene(simpleDepthShader);
        glBindFramebuffer(GL_FRAMEBUFFER, 0);

        // 2. render scene as normal - 应用深度立方体贴图，并正常渲染
        // -------------------------
