...
vec2 ParallaxMapping(vec2 texCoords, vec3 viewDir)
{
    ...
    vec2 prevTexCoords = currentTexCoords + deltaTexCoords;// 反转操作，获取上一个纹理坐标
    ...
    /**
     ...
     https://learnopengl.com/img/advanced-lighting/parallax_mapping_parallax_occlusion_mapping_diagram.png
     */
    float afterDepth  = currentDepthMapValue - currentLayerDepth;// 获取当前采样的深度值和当前层深度的深度差，是一个负值
    float beforeDepth = texture(depthMap, prevTexCoords).r - (currentLayerDepth + layerDepth);// 获取上一个纹理坐标采样的深度值和上一层深度的深度差
 
    /**
     在对（位移的）表面几何进行交叉，找到深度层之后，我们获取交叉前的纹理坐标。然后我们计算来自相应深度层的几何之间的深度之间的距离，并在两个值之间进行插值。
     线性插值的方式是在两个层的纹理坐标之间进行的基础插值。函数最后返回最终的经过插值的纹理坐标。
     
     inear interpolation：线性插值
     
     原文下面有解释公式：
     https://learnopengl.com/Advanced-Lighting/Parallax-Mapping
     */
    // interpolation of texture coordinates
    float weight = afterDepth / (afterDepth - beforeDepth);// 得到一个 [0,1] 的 weight
    // 线性函数
