...
// material parameters - PBR 材质
uniform sampler2D albedoMap;// 反照率(Albedo)纹理，以文素为单位指定平面颜色或者基础反射率
uniform sampler2D normalMap;// 法线贴图纹理，以文素为单位指定每个片段的法线方向，显示每个像素光照细节
uniform sampler2D metallicMap;// 金属(Metallic)贴图纹理，以文素为单位指定平面是不是金属质地的
uniform sampler2D roughnessMap;// 粗糙度(Roughness)贴图纹理，以文素为单位指定平面有多粗糙
uniform sampler2D aoMap;// 环境光遮蔽(Ambient Occlusion)贴图纹理，以文素为单位指定平面的阴影部分
...
void main()
{
    // 反照率(Albedo) - 平面颜色或者基础反射率
    vec3 albedo     = pow(texture(albedoMap, TexCoords).rgb, vec3(2.2));
    // 金属度(Metallic) - 金属质地
    float metallic  = texture(metallicMap, TexCoords).r;
    // 粗糙度(Roughness) - 越粗糙，镜面反射高光更宽阔且更模糊，否则更集中且清晰
    float roughness = texture(roughnessMap, TexCoords).r;
    // 环境光遮蔽(Ambient Occlusion) - 额外阴影因子
