...
struct Texture {
    unsigned int id;// 纹理id，用于绑定纹理
    string type;// 纹理类型
    string path;// 我们储存纹理的路径用于与其它纹理进行比较
};

// 网格结构 https://learnopengl-cn.github.io/img/03/01/assimp_structure.png
/**
...
public:
    // mesh Data
    vector<Vertex>       vertices;// 顶点数据，包含位置，法向量，纹理坐标，切线和副切线（用于法线贴图）
    vector<unsigned int> indices;// 顶点索引
    vector<Texture>      textures;// 加载的纹理对象
    unsigned int VAO;
