...
int main()
{
     ...
     */
    // configure depth map FBO - 配置深度贴图帧缓冲
    // -----------------------
    ...
    // attach depth texture as FBO's depth buffer - 把生成的深度纹理作为帧缓冲的深度缓冲
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 1. render depth of scene to texture (from light's perspective) - 首先从光源视角渲染场景到深度贴图
        // --------------------------------------------------------------
        ...
        // reset viewport - 像往常一样渲染场景，但这次使用深度贴图
        glViewport(0, 0, SCR_WIDTH, SCR_HEIGHT);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        
        // 2. render scene as normal using the generated depth/shadow map  - 使用深度/阴影贴图正常渲染
        shader.use();
        ...
        // render Depth map to quad for visual debugging - 用于debug
        // ---------------------------------------------
//        debugDepthQuad.use();
